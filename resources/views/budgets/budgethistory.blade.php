    ...
    <div class="min-h-screen py-10 w-full flex justify-center" style="background-color: #CCAAFFaa;">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden shadow-sm sm:rounded-lg mb-6 max-w-md mx-auto p-6 text-center animate__animated animate__fadeInDown" style="background-color: #FFFFFF33;">
                @if (Auth::check())
                    <p>Welcome to the Budget History side!</p>
                @else
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center mb-4">
                ...
                <!-- Month Selector -->
                <div class="animate__animated animate__fadeInUp">
                    <form method="GET" action="{{ route('budget.history') }}" class="w-full md:w-2/3 lg:w-1/2">
            ...
            <div class="p-6 form-container mx-auto rounded-lg shadow-lg animate__animated animate__fadeInUp" style="background-color: #FFFFFF77">
                @if($budgets->isEmpty())
                    <p class="text-center text-gray-500 dark:text-gray-400 py-4">No budget history available.</p>
                @else
                    <table class="min-w-full table-auto border-collapse border border-gray-200 dark:border-gray-700" style="background-color: #FFFFFFaa;">
                            ...
                            <tbody>
                                @foreach($budgets as $budget)
                                    <tr class="text-gray-900 dark:text-gray-300" data-id="{{ $budget->item_id }}">
                                        <td class="border border-gray-300 dark:border-gray-700 text-center">
                                            <input type="checkbox" class="row-select" data-id="{{ $budget->id }}" onclick="event.stopPropagation();">
                                        </td>
                                        <td class="px-4 py-2 border-b border-gray-300 dark:border-gray-600 text-center">{{ $budget->item_id }}</td>
                                        <td class="px-4 py-2 border-b border-gray-300 dark:border-gray-600 text-center">{{ $budget->category }}</td>
                                        <td class="px-4 py-2 border-b border-gray-300 dark:border-gray-600">{{ $budget->description }}</td>
                                        <td class="px-4 py-2 border-b border-gray-300 dark:border-gray-600 text-center">{{ $budget->time }}</td>
                                        <td class="px-4 py-2 border-b border-gray-300 dark:border-gray-600 text-center">{{ $budget->date }}</td>
                                        <td class="px-4 py-2 border-b border-gray-300 dark:border-gray-600 text-center">{{ $budget->amount }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                    </table>
            ...
            <!-- Edit Modal -->
            <div id="editBudgetModal" class="fixed z-10 inset-0 overflow-y-auto hidden animate__animated animate__fadeInUp" style="background-color: rgba(0, 0, 0, 0.5);">
                <div class="flex items-center justify-center min-h-screen px-4 py-12">
                    <div class="modal-content bg-white dark:bg-gray-800 shadow-lg rounded-lg max-w-lg w-full p-6">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Edit Budget Item</h2>
                        <form id="editForm" method="POST" enctype="multipart/form-data" action="{{ route('budget.update') }}">
                            @csrf
                            @method('PUT')
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            editForm.addEventListener('submit', (event) => {
                ...
                fetch(editForm.action, {
                    ...
                    .then((data) => {
                        if (data.success) {
                            alert(data.message || 'Budget item updated successfully.');
                            modal.classList.add('hidden'); // Close modal

                            const updatedRow = document.querySelector(`tbody tr[data-id="${formData.get('item_id')}"]`);
                            if (updatedRow) {
                                updatedRow.querySelector('td:nth-child(3)').textContent = formData.get('category');
            ...
            if (searchInput) {
                searchInput.addEventListener('input', () => {
                    ...
                    tableRows.forEach(row => {
                        ...
                        if (category.includes(query) || description.includes(query) || date.includes(query)) {
                            row.style.display = '';
                        } else {
                            row.style.display = 'none';
                        }
