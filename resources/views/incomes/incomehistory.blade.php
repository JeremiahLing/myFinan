    ...
    <div class="min-h-screen py-10 w-full flex justify-center" style="background-color: #CCAAFFaa;">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center mb-4">
                ...
                <!-- Month Selector -->
                <div class="animate__animated animate__fadeInUp">
                    <form method="GET" action="{{ route('income.history') }}" class="w-full md:w-2/3 lg:w-1/2">
                    ...
                    <table class="min-w-full table-auto border-collapse border border-gray-200 dark:border-gray-700" style="background-color: #FFFFFFaa;">
                            ...
                            <tbody>
                                @foreach($incomes as $income)
                                    <tr class="text-gray-900 dark:text-gray-300" data-id="{{ $income->item_id }}">
                                        <td class="border border-gray-300 dark:border-gray-700 text-center">
                                            <input type="checkbox" class="row-select" data-id="{{ $income->id }}" onclick="event.stopPropagation();">
                                        </td>
            ...
            <!-- Edit Modal -->
            <div id="editIncomeModal" class="fixed z-10 inset-0 overflow-y-auto hidden animate__animated animate__fadeInUp" style="background-color: rgba(0, 0, 0, 0.5);">
                <div class="flex items-center justify-center min-h-screen px-4 py-12">
                    <div class="modal-content bg-white dark:bg-gray-800 shadow-lg rounded-lg max-w-lg w-full p-6">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Edit Income Item</h2>
                        <form id="editForm" method="POST" enctype="multipart/form-data" action="{{ route('income.update') }}">
                            @csrf
                            @method('PUT')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            editForm.addEventListener('submit', (event) => {
                ...
                .then((data) => {
                    if (data.success) {
                        alert(data.message || 'Income item updated successfully.');
                        ...
                        alert(data.message || 'Failed to update Income item.');
                    }
                })
                .catch((error) => {
            ...
            document.querySelectorAll('tbody tr').forEach(row => {
                row.addEventListener('click', () => {
                    ...
                    const amount = row.querySelector('td:nth-child(8)').textContent.trim();
                     ...
                     // Format the time to ensure it's valid for type="time"
